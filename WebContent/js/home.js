var choco=angular.module("CHOCO",['ngRoute','ngCookies']);
choco.config(function($routeProvider){
	$routeProvider
...
choco.directive('fileModel', ['$parse', function ($parse) {
    return {
       restrict: 'A',
...
choco.service('fileUpload', ['$http','$location', function ($http,$scope,$location) {
...
choco.run( function ($rootScope, $location,$cookieStore, $http) {
...
choco.controller('registerController', ['$scope', 'fileUpload', function($scope,fileUpload){
    $scope.register = function(){
       ...
       console.log('file is ' );
       console.dir(file);
       var uploadUrl = "http://localhost:2020/CHOCO_BE/fileUpload";
...
choco.service("ChatService", function($q, $timeout) {
...
choco.controller("chatController",function($scope,$http,ChatService,$rootScope)
		{
	...
	$scope.messages = [];
	  ...
	  ChatService.receive().then(null, null, function(message) {
		...
		choco.controller("JobsController",function($scope,$http)
				{
			...
			$scope.addJ=function()
			{
				...
				var res=$http.post("http://localhost:2020/CHOCO_BE/addJobs",jobs);
				...
				choco.controller("loginController",['$cookieStore','$scope','$http','$location','$rootScope',function($cookieStore,$scope,$http,$location,$rootScope)
						{
					...
					$http.post("http://localhost:2020/CHOCO_BE/authenticate",logi).then(function(response)
							{
				...
				choco.controller("blogController",function($scope,$http,$rootScope)	
						{
					$rootScope.userhome=true;
					$rootScope.userforum=true;
					$rootScope.userjobs=true;
					...
					$http.get("http://localhost:2020/CHOCO_BE/viewMyBlogs/"+$rootScope.uname)
							 ...
							 $scope.newBlog={};
								...
								$scope.addBlog=function(newBlog)
								{
									...
									console.log("title:"+dataObj);
									 var res = $http.post('http://localhost:2020/CHOCO_BE/addBlog',dataObj);
									 $http.get("http://localhost:2020/CHOCO_BE/viewMyBlogs/"+$rootScope.uname)
				...
				$scope.saveEdit=function()
				{
					...
					$http.put("http://localhost:2020/CHOCO_BE/updateBlog",edit);
					$http.get("http://localhost:2020/CHOCO_BE/viewMyBlogs/"+$rootScope.uname)
					    .then(function (response) {
				...
				$http.post("http://localhost:2020/CHOCO_BE/deleteBlog",del);
					$http.get("http://localhost:2020/CHOCO_BE/viewMyBlogs/"+$rootScope.uname)
					    .then(function (response) {
				...
				choco.controller("adminBlogController",function($scope,$http,$rootScope)	
						{	
					$rootScope.home=false;
					$rootScope.login=false;
					$rootScope.register=false;
					...
					console.log(" in adminblog controller");
							 ...
							 $http.get("http://localhost:2020/CHOCO_BE/viewBlogs")
							    .then(function (response) {
				...
				choco.controller("adminJobsController",function($scope,$http,$rootScope)	
						{	
					$rootScope.home=false;
					$rootScope.login=false;
					$rootScope.register=false;
					...
					console.log(" in jobs controller");
							 ...
							 $http.get("http://localhost:2020/CHOCO_BE/viewJobs")
							    .then(function (response) {
							 ...
							 $scope.newJobs={};
								...
								$scope.addJobs=function(newJobs)
								{
									...
									console.log("title:"+dataObj);
									 var res = $http.post('http://localhost:2020/CHOCO_BE/addJobs',dataObj);
									 $http.get("http://localhost:2020/CHOCO_BE/viewJobs")
				...
				$scope.saveEdit=function()
				{
					...
					$http.put("http://localhost:2020/CHOCO_BE/updateJob",edit);
					 $http.get("http://localhost:2020/CHOCO_BE/viewJobs")
					    .then(function (response) {
				...
				$http.post("http://localhost:2020/CHOCO_BE/deleteJob",del);
					 $http.get("http://localhost:2020/CHOCO_BE/viewJobs")
					    .then(function (response) {
				...
				choco.controller('logoutController',function($scope,$rootScope,$http,$cookieStore)		
						{
					$http.post("http://localhost:2020/CHOCO_BE/logout/"+$rootScope.uname);
				...
				choco.controller("userJobsController",function($scope,$http,$rootScope)	
						{	
					$rootScope.userhome=true;
					$rootScope.userforum=true;
					$rootScope.userjobs=true;
				...
				choco.controller("adminForumController",function($scope,$http,$rootScope)	
						{	
					$rootScope.home=false;
					$rootScope.login=false;
					$rootScope.register=false;
					...
					console.log(" in adminforum controller");
							 ...
							 $http.get("http://localhost:2020/CHOCO_BE/viewForums")
							    .then(function (response) {
							 ...
							 $scope.newForum={};
								...
								$scope.addForum=function(newForum)
								{
									...
									console.log("title:"+dataObj);
									 var res = $http.post('http://localhost:2020/CHOCO_BE/addForum',dataObj);
									 $http.get("http://localhost:2020/CHOCO_BE/viewForums")
				...
				$scope.approveForum=function()
				{
					...
					$http.put("http://localhost:2020/CHOCO_BE/updateForum",edit);
					 $http.get("http://localhost:2020/CHOCO_BE/viewForums")
					    .then(function (response) {
				...
				choco.controller("userForumController",function($scope,$http,$rootScope)	
						{	
					$rootScope.userhome=true;
					$rootScope.userforum=true;
					$rootScope.userjobs=true;
					...
					console.log(" in forum controller");
							 ...
							 $http.get("http://localhost:2020/CHOCO_BE/viewForum")
							    .then(function (response) {
							 ...
							 $scope.newForum={};
								...
								$scope.answer=function(forum)
								{
									...
									answer:$scope.answerforum.answer
									 ...
									 $http.post('http://localhost:2020/CHOCO_BE/answerForum',ans);
									$http.get("http://localhost:2020/CHOCO_BE/viewForum")
								    .then(function (response) {
								...
								$scope.viewanswers=function(forum)
								{
									...
									console.log("forumid:"+$scope.answerforum.forum_id);
									 $http.get("http://localhost:2020/CHOCO_BE/viewAnswers/"+$scope.answerforum.forum_id)
									    .then(function (response) {
				...
				$http.post("http://localhost:2020/CHOCO_BE/deleteForum",del)
				...
				choco.controller("userHomeController",function($scope,$http,$rootScope)	
						{	
					...
					console.log("name in  findfriends:"+$rootScope.uname);
							 $http.get("http://localhost:2020/CHOCO_BE/findFriends/"+$rootScope.uname)
							    .then(function (response) {
					...
					$scope.addfriend=function(user)
					{
						...
						$http.post("http://localhost:2020/CHOCO_BE/addFriend/",fr);
						
					}
					
						});
				choco.controller("friendslistController",function($scope,$http,$rootScope)	
						{	
					$rootScope.userhome=true;
					$rootScope.userforum=true;
					$rootScope.userjobs=true;
					...
					console.log("name in  friendslist:"+$rootScope.uname);
							 $http.get("http://localhost:2020/CHOCO_BE/viewFriends/"+$rootScope.uname)
							    .then(function (response) {
				...
				choco.controller("allblogsController",function($scope,$http,$rootScope,$route)	
						{	
					$rootScope.userhome=true;
					$rootScope.userforum=true;
					$rootScope.userjobs=true;
					...
					console.log("username in allblog controller:"+$rootScope.uname);
							 $http.get("http://localhost:2020/CHOCO_BE/viewAllBlogs")
							    .then(function (response) {
							 ...
							 { 
						       ...
						       var like=
								...
								console.log("postedby:"+$rootScope.uname);
								 $http.put('http://localhost:2020/CHOCO_BE/updateBlog',like);
								 $http.get("http://localhost:2020/CHOCO_BE/viewBlogs")
								    .then(function (response) {
					 ...
					 $http.get("http://localhost:2020/CHOCO_BE/viewComments/"+$scope.viewcomments.blog_id)
								    .then(function (response) {
						...
						$scope.comment=function(allblogs)
						{
							...
							$http.post('http://localhost:2020/CHOCO_BE/addComment',comment);
							 $http.get("http://localhost:2020/CHOCO_BE/viewBlogs")
							    .then(function (response) {
